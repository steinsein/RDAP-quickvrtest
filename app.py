"""
RDAP 퀵버전 — Streamlit 웹 설문 앱 (수정판 v2)
종교 다양성 태도 프로파일 (Religious Diversity Attitude Profile)

수정 사항 (v2):
  - 결과 해석 대폭 강화: 일관성뿐 아니라 태도 수준, 차별 경향, 영역별 분석 포함
  - Google Sheets 저장 오류 시 디버그 메시지 표시 (개발 단계용)
  - VC·DQ 해석 상세화
    ...
    if cr >= 2:
        if direction < 0:
            ...
            result["discrimination_msgs"].append(
                f"📌 **이슬람에 대한 더 강한 경계 반응**: 동일한 상황에서 이슬람이 관련될 때 "
                f"개신교가 관련될 때보다 더 부정적이거나 조심스러운 반응을 보이고 있습니다. "
                f"이러한 경향은 무슬림 동료와의 관계 형성, 이슬람 문화에 대한 이해, "
                f"모스크 주변 거주 판단 등 실생활에서 미묘한 차별로 나타날 수 있습니다."
            )
        elif direction > 0:
            target = "개신교"
            result["discrimination_msgs"].append(
                f"📌 **개신교에 대한 더 강한 경계 반응**: 동일한 상황에서 개신교가 관련될 때 "
                f"이슬람이 관련될 때보다 더 부정적이거나 조심스러운 반응을 보이고 있습니다. "
                f"한국 사회에서 개신교에 대한 비판적 담론에 영향을 받았을 가능성이 있으며, "
                f"이는 개신교 신자와의 관계에서 선입견으로 작용할 수 있습니다."
            )
